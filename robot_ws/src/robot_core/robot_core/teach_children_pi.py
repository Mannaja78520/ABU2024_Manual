...
class mainRun(Node):
    ...
    def Emergency_StartStop(self):
        ...
        if(pynput.keyboard.KeyCode.char('q') or not IS_EmergencyActive):
            setupServo()
            self.Reset()
            self.EmergencyStop = True
            return
        
        if (pynput.keyboard.KeyCode.char('e') and self.EmergencyStop and IS_EmergencyActive):
            self.Reset(True)
            setupServo()
    ...
    def imu_Read_Action(self):
        ...
        if (pynput.keyboard.KeyCode.char('u')):
            self.yaw = To_Radians(-90)
            self.setpoint = To_Radians(-90)
            return
        
        m = pynput.keyboard.KeyCode.char('i')
        if (not m) :
            self.M_Pressed = False
            return
    ...
    def imuHeading(self): 
        lsb = pynput.keyboard.KeyCode.char('2')
        if (not lsb) :
            self.LSB_Pressed = False
            return
    ...
    def MoveRobot(self):
        ...
        lx = 1 if pynput.keyboard.KeyCode.char('a') else (-1 if pynput.keyboard.KeyCode.char('d') else 0)
        ly = 1 if pynput.keyboard.KeyCode.char('w') else (-1 if pynput.keyboard.KeyCode.char('s') else 0)
        rx = 1 if pynput.keyboard.KeyCode.char('f') else (-1 if pynput.keyboard.KeyCode.char('j') else 0)
    ...
    def Slide_Transform(self):
        b = pynput.keyboard.KeyCode.char('z')
        if (not b) :
            self.B_Pressed = False
            return
        if (self.B_Pressed) : 
    ...
    def UpDown_Transform(self):
        y = pynput.keyboard.KeyCode.char('x')
        if (not y) :
            self.Y_Pressed = False
            return
        if (self.Y_Pressed) :
    ...
    def keepHarvest(self):
        # x = 0.0
        dropDelay = 0.3
        lb = pynput.keyboard.KeyCode.char('i')
        rb = pynput.keyboard.KeyCode.char('o')
        lt = pynput.keyboard.KeyCode.char('k')
        rt = pynput.keyboard.KeyCode.char('l')
        if lb and rb:
            # x = 1.0
            Grip1.angle = 180
            Grip2.angle = 180
    ...
    def keepBall(self):
        # y = 0.0
        a = pynput.keyboard.KeyCode.char('b')
    ...
    def AdjustArm(self):
        # z = 0.0
        x = pynput.keyboard.KeyCode.char('n')
        if (not x) :
            self.X_Pressed = False
            return 
        if (self.X_Pressed) :
